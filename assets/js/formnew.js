...
(function ($) {
    var bootstrapWizardCreate = function (element, options) {
        ...
        this.resetWizard = function () {
            ...
            $('a[data-bs-toggle="tab"]', $navigation).off(
                "shown shown.bs.tab",
                innerTabShown
            );
            ...
            $('a[data-bs-toggle="tab"]', $navigation).on(
                "shown shown.bs.tab",
                innerTabShown
            );
                ...
                t(o ? o : this.options.inputElements)
                    ...
                    .each(function () {
                        ...
                        0 === i.length &&
                            ...
                            o.attr("data-bs-hint") &&
                            (o.after(
                                "<p class='help-block'>" + o.attr("data-bs-hint") + "</p>"
                            ),
                                o.removeAttr("data-bs-hint"));
                        var n = {
            ...
            prototype: {
                init: function () {
                    ...
                    a.each(this.settings.groups, function (b, c) {
                        ...
                        a(this.currentForm)
                            .find("[required], [data-bs-rule-required], .required")
...
$(document).ready(function () {
    ...
    // Code for the Validator
    var $validator = $(".wizzard-card form").validate({
        rules: {
            firstname: {
                required: true,
                minlength: 3
    ...
    // Wizard Initialization
    $(".wizzard-card").bootstrapwizzard({
        tabClass: "nav nav-pills",
        nextSelector: ".btn-next",
        previousSelector: ".btn-previous",

        onNext: function (tab, navigation, index) {
            var $valid = $(".wizzard-card form").valid();
            if (!$valid) {
                $validator.focusInvalid();
                return false;
            }
        ...
        onInit: function (tab, navigation, index) {
            ...
            var $total = navigation.find("li").length;
            $width = 100 / $total;
            var $wizzard = navigation.closest(".wizzard-card");
            ...
            $first_li = navigation.find("li:first-child a").html();
            $moving_div = $('<div class="moving-tab">' + $first_li + "</div>");
            $(".wizzard-card .wizzard-navigation").append($moving_div);
            refreshAnimation($wizzard, index);
            $(".moving-tab").css("transition", "transform 0s");
        },
        ...
        onTabShow: function (tab, navigation, index) {
            ...
            // If it's the last tab then hide the last button and show the finish instead
            if ($current >= $total) {
                $($wizzard).find(".btn-next").hide();
                $($wizzard).find(".btn-finish").show();
            } else {
                $($wizzard).find(".btn-next").show();
                $($wizzard).find(".btn-finish").hide();
            }
    ...
    // Prepare the preview for profile picture
    $("#wizzard-picture").change(function () {
        readURL(this);
    });

    $('[data-bs-toggle="wizzard-radio"]').click(function () {
        wizzard = $(this).closest(".wizzard-card");
        wizzard.find('[data-bs-toggle="wizzard-radio"]').removeClass("active");
        $(this).addClass("active");
        $(wizzard).find('[type="radio"]').removeAttr("checked");
        $(this).find('[type="radio"]').attr("checked", "true");
    });

    $('[data-bs-toggle="wizzard-checkbox"]').click(function () {
        if ($(this).hasClass("active")) {
            $(this).removeClass("active");
            $(this).find('[type="checkbox"]').removeAttr("checked");
        } else {
...
function readURL(input) {
    if (input.files && input.files[0]) {
        ...
        reader.onload = function (e) {
            $("#wizzardPicturePreview").attr("src", e.target.result).fadeIn("slow");
        };
        reader.readAsDataURL(input.files[0]);
    }
}
...
$(window).resize(function () {
    $(".wizzard-card").each(function () {
        $wizzard = $(this);
        index = $wizzard.bootstrapwizzard("currentIndex");
        refreshAnimation($wizzard, index);
...
function refreshAnimation($wizzard, index) {
    total_steps = $wizzard.find("li").length;
    move_distance = $wizzard.width() / total_steps;
    step_width = move_distance;
    move_distance *= index;
    ...
    $wizzard.find(".moving-tab").css("width", step_width);
    $(".moving-tab").css({
        transform: "translate3d(" + move_distance + "px, 0, 0)",
        transition: "all 0.5s cubic-bezier(0.29, 1.42, 0.79, 1)"
    });
